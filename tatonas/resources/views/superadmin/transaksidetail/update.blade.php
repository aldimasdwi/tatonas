...
<div class="w-full lg:w-8/12 px-4 mx-auto mt-6">
  <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
    ...
    <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
      ...
      <form action="/updatetransaksidetail/{{ $td->id }}" method="post">
        @csrf
        ...
        <div class="flex flex-wrap">
          ...
          <div class="w-full lg:w-6/12 px-4">
            <label class="text-dark dark:text-gray-200" for="passwordConfirmation">trans_id	</label>
            <select name="trans_id" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                <option value="{{ $td->trans_id }}">{{ $td->transaksi->id }}</option>
              @foreach ($trans as $hr )
                <option value="{{ $hr->id }}">{{ $hr->id }}</option>
                @endforeach
            </select>
        </div>

        <div class="w-full lg:w-6/12 px-4">
          <label class="text-dark dark:text-gray-200" for="passwordConfirmation">hardware_id	</label>
          <select name="hardware_id" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
            <option value="{{ $td->hardware_id }}">{{ $td->hardware->hardware }}</option>
            @foreach ($har as $se )
            <option value="{{ $se->id }}">{{ $se->hardware }}</option>
            @endforeach
          </select>
      </div>

      <div class="w-full lg:w-6/12 px-4">
        <label class="text-dark dark:text-gray-200" for="passwordConfirmation">sensor_id	</label>
        <select name="sensor_id" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
          <option value="{{ $td->sensor_id }}">{{ $td->sensor->sensor }}</option>
          @foreach ($sen as $se )
    ...
    <div class="w-full lg:w-6/12 px-4">
      <div class="relative w-full mb-3">
        ...
        <input type="text" name="value" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
        value="{{ $td->value }}">
      </div>
    </div>
